...
def webhook():
     ...
     if data['passphrase'] == "ramp":
         ...
         if side=="LONG":
            try:
                ap=client.futures_create_order(symbol='SANDUSDT', side='BUY',positionSide="BOTH",type='MARKET',quantity=136,reduceOnly='false')
                print(ap)
         ...
         if side=="SHORT":
            try:
                ap=client.futures_create_order(symbol='SANDUSDT', side='SELL',positionSide="BOTH",type='MARKET',quantity=136,reduceOnly='false')
                print(ap)
                result = client.futures_get_order(symbol='SANDUSDT',orderId=ap['orderId'])
                pr=float(pr)
                avgp=float(result['avgPrice'])
...
def tl():
     ...
     if data['passphrase'] == "ramp":
         ...
         if side=="LONG":
            try:
                pr=float(pr)
                ap=client.futures_create_order(symbol='LTCUSDT',side=SIDE_BUY, type=ORDER_TYPE_LIMIT, timeInForce=TIME_IN_FORCE_GTC,quantity=1,price='53')
                print(ap)
